...
OpenBoxHub 是纯前端参数化收纳盒设计工具，仓库为 `geekheart/OpenBoxHub`，在线地址为 `https://geekheart.github.io/OpenBoxHub/`。用户在浏览器内配置外盒、整数网格与合并内盒、底板和盒盖，独立编辑零件，查看装配，再下载 STL/ZIP；通过 JSON 文件保存与恢复设计。
...
- ZIP 保持每个零件独立，并包含毫米单位、参数、分组、独立设置、装配变换和打印说明。STL 本身不保存单位或打印配置。
...
界面或资源加载有改动时，在浏览器检查默认参数编辑入口、有效模型、参数修改、错误反馈、全屏与退出、合并与拆分、底板、盖型、视图切换及 STL/ZIP 下载。涉及参数与零件编辑时，还需检查点击选择与下拉选择、独立编辑与恢复自动、外形锁定、JSON 导入导出、失败导入保留当前设计和刷新恢复默认值。涉及动画时，检查展开顺序、展开程度调整、连续快速切换和中途反向，以及 L 形与环形的横移限制。生产检查使用 `dist/` 的 HTTP 静态服务，并验证子路径下 Worker、WASM 和图片加载；仅运行开发服务器不等于部署验证。
