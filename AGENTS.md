...
- 展开顺序为先抬起盒盖，再将内盒竖直移出外盒并完成所需分层，最后横向展开；横移只能在内盒已离开外盒、盒盖保持让位的位置进行。
- 普通等分内盒保持同层。回形、多重嵌套或独立修改的内盒若横移受阻，自动将内侧或受阻内盒升到新的 Z 层，再执行完整横向展开；不得通过缩短展开距离代替分层。检查分层升降、完整横移及回收路径，不得缩放或修改打印几何。
...
- 运动路径覆盖默认等分、L 形、回形、多重嵌套、不同盖型及独立高度；验证普通等分同层、受阻内盒分层后完整展开，并检查连续扫掠结果、阶段边界、模式切换和反向移动。

界面或资源加载有改动时，在浏览器检查默认参数编辑入口、有效模型、参数修改、错误反馈、全屏与退出、合并与拆分、底板、盖型、视图切换及 STEP/STL/ZIP 下载。导出需检查默认整套 STEP、格式切换、三种目标及准备文件期间快速切换格式。涉及参数与零件编辑时，还需检查点击选择与下拉选择、独立编辑与恢复自动、外形锁定、JSON 导入导出、失败导入保留当前设计和刷新恢复默认值。涉及动画时，检查展开顺序、展开程度调整、连续快速切换和中途反向，以及回形与多重嵌套的自动分层；检查盒盖文字按钮、隐藏后的取景和导出仍包含盒盖。生产检查使用 `dist/` 的 HTTP 静态服务，并验证子路径下 Worker、WASM 和图片加载；仅运行开发服务器不等于部署验证。
