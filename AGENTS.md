...
界面或资源加载有改动时，在浏览器检查有效模型、参数修改、错误反馈、全屏与退出、合并与拆分、底板、盖型、视图切换及 STL/ZIP 下载。涉及参数与零件编辑时，还需检查点击选择与下拉选择、独立编辑与恢复自动、外形锁定、JSON 导入导出、失败导入保留当前设计和刷新恢复默认值。生产检查使用 `dist/` 的 HTTP 静态服务，并验证子路径下 Worker、WASM 和图片加载；仅运行开发服务器不等于部署验证。
