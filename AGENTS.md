...
- 普通等分内盒保持同层。回形、多重嵌套、U/C 形开口凹槽中的内盒及横移受阻的内盒，自动升到新的 Z 层，再执行完整横向展开；同中心、无相对径向位移的凹槽内盒也必须分层。不得通过缩短展开距离代替分层；检查分层升降、完整横移及回收路径，不得缩放或修改打印几何。
- 爆炸程度默认 `100%`。「组合」「开盖」「爆炸」共用运动路径参数。切换模式或中途反向时经过共用安全节点，不能对任意起止姿态直接插值而跳过已检查的路径。
